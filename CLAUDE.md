...
- **Caché offline de Firestore** (`persistentLocalCache` + `persistentMultipleTabManager` en `config.js`): da resiliencia en red pobre (getDocs cae a caché en vez de colgarse). OJO: `getDocs` es "servidor primero" online, así que esto SOLO no vuelve instantánea la carga online.
