...
- **Conciliación bajo demanda con la API de Zoho** (lo pidió el dueño: "el sistema debería hacerla, no manual"): GK **consulta** a Zoho el estado real de las facturas en vez de esperar el webhook. Botón "Actualizar facturas desde Zoho" en AdminPanel → Integraciones (sección 2). Callables `reconciliarFacturasZoho`/`probarConexionZoho`/`guardarCredencialesZoho` (`zohoReconcile.js`); cliente API v3 en `zohoApi.js` (OAuth self-client, refresh_token). Credenciales en `zoho_secure/creds` (regla Firestore `allow read,write: if false` — solo Admin SDK). Reusa `upsertFacturaFromZoho` (`facturaSync.js`), la MISMA lógica del webhook (extraída para evitar drift): resuelve vendedor, período/recuperada, congela tasa-cohorte y calcula comisión al detectar el pago. **Idempotente** (no re-genera comisión si ya está pagada) y seguro de correr N veces. El listado de Zoho no trae `line_items`, así que el upsert **conserva las unidades** ya guardadas cuando faltan (nunca las borra; la comisión se calcula sobre el monto). Requiere que el dueño genere el self-client de Zoho (scope `ZohoBooks.invoices.READ`). Acción `conciliarPago` en `gestionarFacturaVendedor` como respaldo manual factura por factura.
