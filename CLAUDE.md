...
- **3.6 — Meta de cobranza + Bono Cobranza** ✅ (modelo de PUNTUALIDAD, no volumen: a tiempo = cobrar dentro de `vencimiento + cobranzaGraciaDias`; Bono Cobranza si `% a tiempo ≥ cobranzaUmbral`) + **Cuentas Recuperadas** ✅ (5% flat en facturas con fecha < `fechaIngreso`, no cuentan a la meta). `commissionConfig` con `bajaLabel`/`bajaActiva`/nombres de tier editables; PDF de propuesta.
