...
export const LiquidacionesManagement = ({ vendedores: vendedoresProp, lockedVendedorId } = {}) => {
    ...
    // Modo FLUJO GUIADO: el layout fija el vendedor (paso ② Liquidar).
    useEffect(() => {
        if (lockedVendedorId && lockedVendedorId !== vendedorId) onSelectVendedor(lockedVendedorId);
...
export const ConciliacionFacturas = ({ vendedores: vendedoresProp, lockedVendedorId, onLiquidar } = {}) => {
    const [vendedoresLocal, setVendedores] = useState([]);
    const vendedores = (vendedoresProp && vendedoresProp.length) ? vendedoresProp : vendedoresLocal;
    const [vendedorId, setVendedorId] = useState('');
    ...
    // Modo FLUJO GUIADO: el layout del administrador fija el vendedor (paso ①
    // Conciliar) — se pre-selecciona y se oculta el desplegable.
    useEffect(() => {
        if (lockedVendedorId && lockedVendedorId !== vendedorId) onSelect(lockedVendedorId);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [lockedVendedorId]);
                        ...
                        <div className="bg-brand-blue/5 border border-brand-blue/20 rounded-xl p-4 mb-3">
                            ...
                            {onLiquidar && (
                                <button onClick={onLiquidar} className="w-full mt-3 flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm px-4 py-3 rounded-lg">
                                    Continuar a liquidar <span aria-hidden>→</span>
                                </button>
                            )}
                        </div>
                    )}
...
export const ComisionesDashboard = ({ vendedores: vendedoresProp, onPagar } = {}) => {
                            ...
                            <div key={r.vendedor.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                                ...
                                {onPagar && (
                                    <div className="px-4 pb-3 -mt-1">
                                        <button
                                            onClick={() => onPagar(r.vendedor.id)}
                                            className="w-full flex items-center justify-center gap-2 bg-[#0D2B4C] hover:opacity-90 text-white font-bold text-sm px-4 py-2.5 rounded-lg"
                                        >
                                            Conciliar y liquidar <span aria-hidden>→</span>
                                        </button>
                                    </div>
                                )}
