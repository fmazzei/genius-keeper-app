...
const Planner = ({ role, geniusTasks = [], allPossibleStops = [], agenda: initialAgenda, onSelectPos }) => {
    ...
    const assignedPosIdsInAgenda = useMemo(() => {
        ...
        if (agenda && agenda.days) {
            Object.values(agenda.days).forEach(dayPlan => {
                dayPlan.forEach(stop => ids.add(stop.id));
            });
        }
        return ids;
    }, [agenda]);
    ...
    useEffect(() => {
        navigator.geolocation.getCurrentPosition(
            ...
            () => console.warn("No se pudo obtener la ubicación para la navegación."),
            { enableHighAccuracy: true }
        );
    }, []);

    useEffect(() => { if (initialAgenda) setAgenda(initialAgenda); }, [initialAgenda]);
    ...
    const handleToggleStopInPlan = useCallback((stop) => {
        ...
        if (isInCurrentList) {
            ...
            if (isAgenda && assignedPosIdsInAgenda.has(stop.id)) {
                if (window.confirm("Este PDV ya está asignado a otro día. ¿Deseas agregarlo de todas formas?")) {
    ...
    }, [activeTab, currentPlan, agendaDayPlan, selectedDay, assignedPosIdsInAgenda]);
    ...
    const generateSpontaneousPlan = useCallback((stopCount, depotId) => {
        ...
        startPromise.then(startLocation => {
            ...
            const sorted = masterStopList
                .filter(s => s.isSuggested && s.location)
                .sort((a, b) => haversineDistance(a.location, startLocation) - haversineDistance(b.location, startLocation));
    ...
    const handleDelegate = useCallback(async (delegateToUser) => {
        ...
        try {
            await addDoc(collection(db, "delegated_tasks"), {
                ...
                delegatedToId: "anonymous_merchandiser_uid",
    ...
    return (
        // ✅ LAYOUT CORREGIDO: Este es el contenedor principal que ocupa toda la altura.
        <div className="h-full flex flex-col bg-white relative">
            
            {/* El contenido de la pestaña activa se renderiza aquí y se expande */}
                ...
                {activeTab === 'hoy' && (
                    <div className="p-4 flex flex-col h-full bg-slate-50">
                        <div className="flex-grow">
                            ...
                            <div className="overflow-y-auto my-2 bg-white rounded-lg border p-2 min-h-[200px]">
                                {currentPlan.length > 0 ? (
                                    <ol className="list-decimal list-inside space-y-2">{currentPlan.map((stop, index) => (<li key={stop.id} className="font-semibold text-slate-700 p-2 border-b group flex justify-between items-center"><span onClick={() => onSelectPos(stop)} className="cursor-pointer flex-1 truncate">{index + 1}. {stop.name}</span><div className='flex items-center opacity-0 group-hover:opacity-100 transition-opacity'><button onClick={() => setStopToDelegate(stop)} title="Delegar parada" className="p-1 text-slate-400 hover:text-blue-600"><Users size={16} /></button><button onClick={() => handleToggleStopInPlan(stop)} title="Quitar del plan" className="p-1 text-slate-400 hover:text-red-600"><Trash2 size={16} /></button></div></li>))}</ol>
                                ) : ( <div className="h-full flex flex-col justify-center items-center text-center text-slate-500"><Compass size={48} className="mx-auto text-slate-300 mb-4" /><h4 className="font-bold">Tu plan está vacío</h4><p className="text-sm">Usa los botones de arriba para empezar.</p></div> )}
                            </div>
                        </div>
            ...
            <AddStopModal 
                isOpen={isAddStopViewOpen} 
                onClose={() => setAddStopViewOpen(false)} 
                masterStopList={masterStopList} 
                activePlan={activeTab === 'hoy' ? currentPlan : agendaDayPlan} 
                onToggle={handleToggleStopInPlan} 
                onIntelligentInsert={handleIntelligentInsert} 
                onDelegate={setStopToDelegate}
                assignedPosIdsInAgenda={assignedPosIdsInAgenda}
                isAgendaView={activeTab === 'agenda'}
            />
            <SpontaneousPlanModal isOpen={isSpontaneousModalOpen} onClose={() => setIsSpontaneousModalOpen(false)} onGenerate={generateSpontaneousPlan} depots={depots} />
            <MoveStopModal isOpen={!!stopToMove} onClose={() => setStopToMove(null)} fromDay={selectedDay} onMove={handleMoveStopInAgenda} />
            <DelegateModal isOpen={!!stopToDelegate} onClose={() => setStopToDelegate(null)} onDelegate={handleDelegate} stop={stopToDelegate} />
        </div>
    );
};
