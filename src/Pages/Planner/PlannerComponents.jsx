// RUTA: src/Pages/Planner/PlannerComponents.jsx

import React, { memo, useCallback } from 'react';
import { ChevronDown, CheckSquare, Square, Store, Building, Sparkles, Share2 } from 'lucide-react';
...
export const StopBankItem = memo(({ stop, isSelected, onToggle, onDelegate, assignedPosIdsInAgenda, isAgendaView }) => {
    const handleToggle = useCallback(() => onToggle(stop), [onToggle, stop]);
    const handleDelegate = useCallback((e) => {
        e.stopPropagation(); // Evita que el clic en delegar también seleccione el item
        onDelegate(stop);
    }, [onDelegate, stop]);

    // Lógica para mostrar el indicador:
    // - Debe estar en la vista de Agenda.
    // - El PDV debe estar en la lista de asignados.
    // - No debe estar ya seleccionado para el día actual (el checkmark tiene prioridad).
    const isAssignedElsewhere = isAgendaView && assignedPosIdsInAgenda.has(stop.id) && !isSelected;
            ...
            <div onClick={handleToggle} className="flex-1 flex items-center gap-3 cursor-pointer">
                ...
                <div>
                    <p className="text-sm font-semibold flex items-center gap-2">
                        ...
                        {stop.name}
                        {/* Indicador visual que muestra un punto azul si ya está asignado */}
                        {isAssignedElsewhere && (
                            <span className="w-2 h-2 bg-blue-500 rounded-full" title="Ya asignado en la agenda semanal"></span>
                        )}
                    </p>
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-sm animate-fade-in-up">
                ...
                <label className="font-semibold">¿Cuántas paradas quieres visitar?</label>
                <input type="number" value={count} onChange={e => setCount(parseInt(e.target.value) || 1)} className="w-full p-3 border-2 text-center text-2xl font-bold rounded-lg my-2"/>
                <label className="font-semibold mt-2">Punto de Partida (Opcional)</label>
                <select value={depot} onChange={e => setDepot(e.target.value)} className="w-full p-3 border-2 rounded-lg mt-1 bg-white">
                    <option value="">Mi ubicación actual</option>
                    {depots.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                </select>
                <button onClick={() => onGenerate(count, depot)} className="w-full mt-4 bg-brand-yellow text-black font-bold py-3 rounded-lg">Crear Mi Ruta</button>
                <button onClick={onClose} className="w-full mt-2 text-sm text-slate-500">Cancelar</button>
            </div>
        </div>
    );
};
...
export const DelegateModal = ({ isOpen, onClose, onDelegate, stop }) => {
    if (!isOpen) return null;
    const [delegateTo, setDelegateTo] = React.useState('Carolina Ramírez');
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-sm animate-fade-in-up">
