...
// SOLUCIÓN: Importar los componentes centralizados
import { Accordion, StopBankItem } from '../Pages/Planner/PlannerComponents.jsx';

const AddStopModal = ({ isOpen, onClose, masterStopList, activePlan, onToggle, onIntelligentInsert, onDelegate }) => {
    ...
    const groupedFilteredPos = useMemo(() => {
        ...
        (filteredList || []).forEach(pos => {
            if (pos.isSuggested) {
                groups.suggestions.items.push(pos);
                return;
            }
            if (pos.type === 'depot') {
                groups.depots.items.push(pos);
                return;
            }
            const key = pos.chain || 'Individuales';
            if (!groups[key]) {
                groups[key] = { title: key, icon: <Store size={16}/>, items: [] };
            }
            groups[key].items.push(pos);
        });
        
        const sortedChains = Object.keys(groups)
            .filter(key => key !== 'suggestions' && key !== 'depots')
            .sort((a, b) => a.localeCompare(b));
            
        return [
            groups.suggestions,
            groups.depots,
            ...sortedChains.map(key => groups[key])
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
            <div className="bg-slate-50 rounded-lg w-full max-w-2xl h-[90vh] flex flex-col animate-fade-in-up relative">
                ...
                <main className="flex-1 overflow-y-auto p-4 pt-12 space-y-4 relative">
                    ...
                    <h2 className="text-xl font-bold text-center mb-4">Añadir Paradas</h2>
                    ...
                    <div className="bg-white p-2 rounded-lg border">
                        ...
                        {showFilters && (
                            <div className="p-2 pt-4 space-y-3 border-t mt-2 animate-fade-in">
                                <select onChange={e => handleFilterChange(setZoneFilter, e.target.value)} className="w-full p-2 border rounded-md bg-white">
                                    <option value="Todas">Todas las Zonas</option>
                                    {zones.map(z => <option key={z} value={z}>{z}</option>)}
                                </select>
                                <select onChange={e => handleFilterChange(setTypeFilter, e.target.value)} className="w-full p-2 border rounded-md bg-white">
                                    <option value="Todas">Todos los Tipos</option>
                                    <option value="Cadenas">Cadenas</option>
                                    <option value="Individuales">Individuales</option>
                                </select>
                                <select onChange={e => handleFilterChange(setAlertFilter, e.target.value)} className="w-full p-2 border rounded-md bg-white">
                    ...
                    {groupedFilteredPos.map(group => (
                        <Accordion 
                            ...
                            icon={group.icon} 
                            isOpen={!!openAccordions[group.title]} 
