...
// que el Estado de Cuenta (monoespaciado, logo GK, barra verde), pero explicando
// CADA número con evidencia de facturas: por qué el vendedor está en tal nivel,
// qué facturas ganaron el Bono Cobranza, y semana por semana la Activación
// (clientes activados con ≥N uds y las facturas que lo prueban). Puede ocupar
// más de una hoja carta (es un documento de auditoría, no un flyer).
...
export default function LiquidacionDetalladaDoc({ desglose, vendedorName = 'Vendedor', onClose }) {
            ...
            <div className="flex-1 p-3 sm:p-6 flex justify-center">
                <div id="gk-liq-sheet" className="bg-white w-full max-w-[720px] shadow-2xl self-start text-[12.5px] leading-relaxed text-[#111]" style={{ fontFamily: MONO }}>
                    ...
                    <div className="px-8 pt-4 text-[12px]">
                        ...
                        <div className="flex justify-between"><span className="text-slate-600">Período:</span><span>Mes {d.mes} · {d.rango} · {d.anio}</span></div>
                        <div className="flex justify-between"><span className="text-slate-600">Estado:</span><span>{d.congelado ? 'Cerrado · congelado' : d.cerrado ? 'Cerrado (provisional)' : 'En curso (provisional)'}</span></div>
                    ...
                    <div className="px-8 gk-sec">
                        ...
                        <p className="text-[11px] text-slate-600 mb-1">Colocaste <b>{uds(d.unidades)}</b> de <b>{uds(d.metaMensual)}</b> uds ({d.pct}% de tu meta) → <b>Nivel {d.nivel}</b> (tasa {d.tasa}%).</p>
                    ...
                    <div className="px-8 gk-sec">
                        ...
                        {d.facturasATiempo.length === 0 ? (
                            <p className="text-[11px] text-slate-500">Aún no hay facturas cobradas a tiempo en este período.</p>
                        ) : (
                            <>
                    ...
                    <div className="px-8 gk-sec">
                        ...
                        {d.carteraSize === 0 ? (
                            ...
                            <>
                                <p className="text-[11px] text-slate-600 mb-2">Objetivo: activar ≥{d.actThreshold}% de tu cartera ({d.objetivo} de {d.carteraSize} clientes) con ≥{d.actMinUnits} uds cada semana.</p>
                                {d.semanas.map(w => (
