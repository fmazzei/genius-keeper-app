...
// --- SUB-COMPONENTE: Modal de Confirmación con Mapa (Reutilizado) ---
const VerificationModal = ({ merchandiserCoords, geniusCoords, distance, onConfirm, onCorrect, onClose }) => {
    ...
    // que tenga la "Maps Static API" habilitada.
    const STATIC_MAPS_API_KEY = "TU_API_KEY_DE_GOOGLE_MAPS_AQUÍ";

    const mapUrl = `https://maps.googleapis.com/maps/api/staticmap?size=600x300&maptype=roadmap&markers=color:blue%7Clabel:M%7C${merchandiserCoords.lat},${merchandiserCoords.lng}&markers=color:red%7Clabel:G%7C${geniusCoords.lat},${geniusCoords.lng}&key=${STATIC_MAPS_API_KEY}`;
        ...
        <Modal isOpen={true} onClose={onClose} title="Confirmar Ubicación del PDV">
            <div className="p-4 text-center">
                ...
                <p className="text-slate-600 my-2">
                    La ubicación que capturaste (Pin Azul) y la ubicación encontrada por Genius según la dirección (Pin Rojo) tienen una diferencia de <strong>{Math.round(distance)} metros</strong>.
                </p>
                <img src={mapUrl} alt="Mapa de comparación de ubicaciones" className="rounded-lg my-4 border" />
                <p className="text-sm text-slate-500 mb-6">Por favor, revisa el mapa y confirma cuál es la ubicación correcta o corrige la dirección.</p>
                <div className="flex flex-col sm:flex-row justify-center gap-3">
                    <button onClick={onConfirm} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-brand-blue text-white rounded-lg font-bold">
                        <Check size={20}/> Usar Mi Ubicación (Pin Azul)
                    </button>
                    <button onClick={onCorrect} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-slate-200 text-slate-800 rounded-lg font-semibold">
                        <Edit3 size={20}/> Corregir Dirección
                    </button>
                </div>
            </div>
        </Modal>
...
const UpdatePosGpsModal = ({ pos, onClose, onConfirm }) => {
    ...
    const savePosUpdateToFirestore = async (coordinatesToSave) => {
        ...
        try {
            ...
            await updateDoc(posRef, {
                address: address, // Actualizamos también la dirección por si la corrigió
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const result = await geocodeAddressByGenius({ address: `${pos.name}, ${address}, ${pos.zone}, Caracas, Venezuela` });
    ...
    if (step === 'confirm') {
        return (
            <VerificationModal
                ...
                distance={distance}
                onConfirm={() => savePosUpdateToFirestore(merchandiserLocation)}
