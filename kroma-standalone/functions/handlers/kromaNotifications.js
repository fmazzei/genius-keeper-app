// Cloud Function: notificaciones push de temporizadores de producción (holds).
// Corre cada 2 minutos y envía FCM a los dispositivos del operario cuando el
// temporizador de un bloque está por finalizar.
...
exports.kromaHoldNotifier = onSchedule({
    ...
    timeZone:  "America/Caracas",  // ⚙️ ajusta a la zona horaria de la quesería
    region:    "us-central1",
}, async () => {
    ...
    await Promise.all(due.map(async (docSnap) => {
        ...
        // Tokens FCM del operario (guardados por la app en kroma_users/{id}/tokens)
        const tokensSnap = await db
            .collection("kroma_users").doc(userId)
            .collection("tokens").get();

        // Marcar como disparada aunque no haya tokens (evita re-intentos)
        ...
        try {
            ...
            result.responses.forEach((r, i) => {
                if (r.error && [
                    ...
                    invalidos.push(
                        db.collection("kroma_users").doc(userId)
