                ...
                <div className="border-t border-slate-700 px-4 pb-4 pt-3 space-y-3">
                    ...
                    {needsGKSync && (
                        ...
                        >
                            ...
                            {syncingGK ? 'Sincronizando inventario…' : 'Sincronizar inventario de destino'}
                        </button>
                    )}
...
export default function DespachoPage() {
    ...
    const markEntregado = async (despacho) => {
        ...
        try {
            ...
            // Precarga los almacenes y el inventario de destino para sincronizar al entregar
            const [almSnap, invComSnap] = await Promise.all([
                getDocs(collection(db, 'kroma_almacenes_destino')),
            ...
            for (const linea of (despacho.lineas || [])) {
                ...
                if (isCaracasDest && caracasWh) {
                    ...
                    // Sincroniza al inventario de destino (kroma_inventario_destino)
                    ...
                    if (existing) {
                        await updateDoc(doc(db, 'kroma_inventario_destino', existing.id), {
                        ...
                        const newRef = await addDoc(collection(db, 'kroma_inventario_destino'), {
    ...
    // Escribe las cantidades entregadas en kroma_inventario_destino sin tocar kroma_inventory_pt.
